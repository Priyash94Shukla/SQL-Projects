...
LIMIT 10;
-- checking if Data if imported completely
...
-- Checking the Null Value (in one query)
SELECT * FROM retail_sales
WHERE 
...
-- Deleting the NULL values (Except for age column) 
...
-- Define all the unique category names.
...
SELECT year, month, ROUND(avg_sales::numeric,2)
FROM
(
SELECT 
